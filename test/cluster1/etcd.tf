...
module "etcd_node_01" {
  source = "git::https://github.com/icereval/kubit.git?ref=feature/multi-cluster//modules/etcd_node"
  ...
  internal_domain_zone_id = "${var.domain_zone_ids["internal"]}"
...
module "etcd_dns" {
  source = "git::https://github.com/icereval/kubit.git?ref=feature/multi-cluster//modules/etcd_dns"
  cluster_name = "${var.cluster["name"]}"
  etcd_node_fqdns = [
